...
function checkedPawnStep(obj,board,prev){
    switch(obj.color){
        case "white": return checkedPawnStepWhite(obj,board,prev);
        case "black": return checkedPawnStepBlack(obj,board,prev);
        default: return[]
    }
...
function checkedPawnStepWhite(obj,board,prev){
    ...
    if(pos[0] >=0 && pos[1] <=7){
        ...
        arr.map((element) =>{
            if( (board[pos[0]][pos[1]].color === board[element[0]][element[1]].color || board[pos[0]][pos[1]].color !== board[element[0]][element[1]].color) && board[element[0]][element[1]].name != "" ){
        ...
        if(board[pos[0]-1][pos[1]+1]?.name != "" && board[pos[0]][pos[1]]?.color != board[pos[0]-1][pos[1]+1]?.color){
...
function checkedKnightStep(obj,board,prev,isBool){
    ...
    let arr = res.filter(item => (item[0] >= 0  && item[0] <=7) && (item[1] >= 0  && item[1] <=7) )
    res = [...arr]
    ...
    for (let i = pos[0] - 1 ; i >= n; i--) {
        for (let j = pos[1]+1; j <=7;j++ ) { 
            ...
            if(j == ex) {
                ...
                res.push([i,j])
                if(board[pos[0]][pos[1]].color !== board[i][j].color && board[i][j].name != "" ){
                    bool = true;
                    break;
                }
            }
...
function checkedRookStep(obj,board,prev){
    ...
    for (let i = pos[0] - 1; i >= 0; i--) {
        ...
        res.push([i,jt])
        if(board[pos[0]][pos[1]].color !== board[i][jt].color && board[i][jt].name != "" ){
    ...
    for (let j = pos[1] + 1; j <= 7; j++) {
        ...
        res.push([it,j])
        if(board[pos[0]][pos[1]].color !== board[it][j].color && board[it][j].name != "" ){
...
const stepDiagonal = (obj,board,prev, isBool) => {
    let pos = isBool ? findPositionForShah(obj,board,prev):  findPosition(obj,board,prev);
        ...
        for (let j = pos[1]-1; j >= 0; j--) {  
            ...
            if(j == (ex)){
                if(isBool){
                    ...
                    if(obj.color !== board[i][j].color && board[i][j].name != "" ){
                        bool = true;
                        break;
                    }  
                } else{
                    if(obj.color === board[i][j].color && board[i][j].name != "king"){
                        bool = true;
                        break;
                    }
                    res.push([i,j])
...
function checkedKnightStepForShah(obj,board,prev){
    ...
    res.push([[pos[0]-1,pos[1]+2]])
    res.push([[pos[0]-1,pos[1]-2]])
    let arr = res.filter(item => (item[0][0] >= 0  && item[0][0] <=7) && (item[0][1] >= 0  && item[0][1] <=7) )
    res = [...arr]
    arr.map((element,index) =>{
...
const stepDirect = (obj,board,prev,isBool) =>{
    ...
    for (let i = pos[0] - 1; i >= 0; i--) {
        if(isBool){
            ...
            if(obj.color === board[i][jt].color && board[i][jt].name !="king"){
                break;
            }
            res.push([i,jt])
    ...
    for (let j = pos[1] + 1; j <= 7; j++) {
        if(isBool){
            ...
            if(obj.color === board[it][j].color && board[it][j].name !="king"){
...
export const checkKingAroundAndEnemyFigures = (obj,board,prev) =>{
    ...
    for (let i = 0; i < partArray.length; i++) {
        ...
        if(board[partArray[i][0]][partArray[i][1]].color != board[pos[0]][pos[1]].color && board[partArray[i][0]][partArray[i][1]].color  != ""){
    ...
    return isContains(availableStep,pos) ? intersection(curentAvailable, availableStep) : []
}
...
function checkedPawnStepForShah(obj,board,prev){
    switch(obj.color){
        case "white": return checkedPawnStepWhiteForShah(obj,board,prev);
        case "black": return checkedPawnStepBlackForShah(obj,board,prev);
        default: return[]
    }
...
function checkedPawnStepWhiteForShah(obj,board,prev){
    ...
    if(pos[0] >=0 && pos[1] <=7){
        ...
        arr.map((element) =>{
            if( board[pos[0]][pos[1]].color === board[element[0]][element[1]].color || board[pos[0]][pos[1]].color !== board[element[0]][element[1]].color){
                let index = res.indexOf(element)
                res.splice(index,1) 
                if(board[element[0]][element[1]].id.includes('3')){
                    res.splice(index,1) 
        ...
        if( board[pos[0]][pos[1]]?.color != board[pos[0]-1][pos[1]+1]?.color){
            res.push([pos[0]-1,pos[1]+1])
        }
        if( board[pos[0]][pos[1]]?.color != board[pos[0]-1][pos[1]-1]?.color){
            res.push([pos[0]-1,pos[1]-1])
...
export const checkKingCantStep = (obj,board,prev) =>{
    ...
    kingStep.forEach(el =>{
        ...
        res = res.concat(checkedKnightStepForShah(objCurField,board,prev,true))
...
const findEnemyFigureIsAvailable = (res, pos,board,prev) =>{
    let posFigure = {}
    let arrPosFigure = [];
    for (let row = 0; row < res.length; row++) {
        for (let col = 0; col < res[row].length; col++) {
            if(board[res[row][col][0]][res[row][col][1]].color != board[pos[0]][pos[1]].color && board[res[row][col][0]][res[row][col][1]].color  != ""){
    ...
    arrPosFigure.map(el =>{
        ...
        if(el.name === "pawn"){
           ...
           let res = availableStep
            res.forEach((element)=>{
                if(element[1]%2 === pos[1]%2 && posPawm[1]%2 === pos[1]%2){
...
export const checkUnionOverlaps = (overlaps, board) =>{
    let result = stepDiagonal(objKing,board,prev, true);
    let res = result.concat(stepDirect(objKing,board,prev, true));
    res = res.concat(checkedKnightStepForShah(obj,board,prev,true))
}
