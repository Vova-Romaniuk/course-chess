...
function FullBoard() {
	...
	const replace = () => {
		if (active) {
			if (
				...
				setPrev(next);
				setLimitStep(
					selectFigure(checkObj, board, prev, false, unionOverlaps)
				);
				setNext([]);
				setActive(false);
			}
	...
	const selectTurn = (obj, indexColumn, indexRow) => {
		...
		if (unionOverlaps.length != 0) {
			...
			for (let i = 0; i < unionOverlaps.length; i++) {
				if (
					unionOverlaps[i][0] === indexRow &&
					unionOverlaps[i][1] === indexColumn
				) {
					bool = false;
				}
			}
			...
			if (
				posUnionKing[0] === indexRow &&
				posUnionKing[1] === indexColumn
			) {
				bool = false;
			}
			if (posKing[0] === indexRow && posKing[1] === indexColumn) {
	...
	const boxColor = (column, checkObj, indexRow, indexColumn) => {
		...
		limit?.forEach((element) => {
			if (
				element[0] == indexRow &&
				element[1] == indexColumn &&
				!selectTurn(column, indexColumn, indexRow)
			) {
